...
layout = dbc.Card([
                ...
                dbc.Col([
                    dbc.Card([
                        ...
                        dbc.CardBody([
                            ...
                            html.P(
                                "Esse projeto é um protótipo, o botão de adicionar um novo perfil esta desativado "
                                "momentaneamente!",
                                className="card-text",
                            ),
    ...
    ThemeChangerAIO(aio_id="theme", radio_props={"value": dbc.themes.MINTY})
...
def add_category(n, n2, txt, check_delete, data):
    cat_despesas = list(data["Categoria"].values())
...
def add_category(n, n2, txt, check_delete, data):
    cat_receitas = list(data["Categoria"].values())
...
def salve_form_despesa(n, valor, switches, categoria, date, descricao, dict_despesas):
    ...
    if n and not (valor == "" or valor is None):
        value_rounded = round(float(valor), 2)
        ...
        df_despesas.loc[df_despesas.shape[0]] = [value_rounded, recebido, fixo, datetime, categoria, descricao]
